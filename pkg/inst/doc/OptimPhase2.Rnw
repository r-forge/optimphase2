...
{\it OptimDes} finds a two-stage design for a time to event endpoint with potential stopping for futility after the 
first stage. It implements the Case and Morgan (2003) generalization of the Simon (1989) two-stage design for comparing 
a treatment to a known standard. The design minimizes either the expected duration of accrual (EDA), expected sample 
size (ES), or the expected total study length (ETSL). 
...
the accrual distribution affect power (and alpha level in some settings), see Case and Morgan (2003)). 
...
pdf("object2.pdf",height=5, width=5)
plot(object2)
dev.off()
@
...
<<>>=
object3 <- np.OptimDes(B.init,m.init,alpha,beta,param,x,pt=1.1,target="ETSL")
print(object3)
@
...
of two-stage designs from function {\it OptimDes} with the targeted nominal values. It can also be used to assess 
...
an exact test for the second stage test is \Sexpr{sim1["alphaExact"]}.
